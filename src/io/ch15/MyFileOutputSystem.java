...
public class MyFileOutputSystem {
    public static void main(String[] args) {
        ...
        // 파일이 없으면 새로생성, 있으면 덮어쓰기
        // FileOutputStream fos = new FileOutputSystem("output.txt")
        // FileOutputStream fos = new FileOutputSystem("output.txt, true) >> Append 모드
        // 기존 파일에 내용이 있다면 true 뒤에 이어쓰기, false >> 덮어쓰기
        try (FileOutputStream fos = new FileOutputStream("output.txt", false)) {
            // 문자열은 FileOutputStream 으로 직접 쓸 수 없다.
            // 문자열은 >> byte 배열로 변환해서 넣어줘야함
            // 문자열을 바이트 배열로 변환해서 임시 변수에 담아둠
            //byte[]  dataBytes = data.getBytes();
            fos.write(data.getBytes());
            // fos.flush(); 변기에 물을 내린다...와 같은..
        ...
        // FileOutputSystem으로 바이트를 썼지만 보여주는 방식은 에디터가 결정한다.
